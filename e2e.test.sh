...
function recreateComposite() {
  ...
  assertCurl 200 "curl -X DELETE http://$HOST:$PORT/composite/product/${productId} -s"
  curl -X POST http://$HOST:$PORT/composite/product -H "Content-Type: application/json" --data "$composite"
}
...
waitForService curl -X DELETE http://$HOST:$PORT/composite/product/$PROD_ID_NOT_FOUND
